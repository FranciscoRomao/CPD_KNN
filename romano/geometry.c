...
long furthest_point_from_coords(int n_dims, long n_points, double **pts, double *base_coords)
{
    ...
    //printf("num: %d\n", n_threads);
    long* ids = (long *)malloc(n_threads * sizeof(long));
    double* values = (double *)malloc(n_threads * sizeof(double));

    long idx_newpt = 0;
    long i;

    #pragma omp parallel for reduction(max:max_dist) private(myId, curr_dist, flag)
    for (i = 0; i < n_points; i++)
    {
        ...
        curr_dist = squared_distance(n_dims, base_coords, pts[i]);
        printf("Thread: %d a comparar: %lf com max: %lf\n", myId, curr_dist, max_dist);
        if(curr_dist > max_dist)
        {
            max_dist = curr_dist;
            values[myId-1] = curr_dist;
            ids[myId-1] = i;
        }
    }

    i = find_idx_from_value(values, n_threads, max_dist);
    idx_newpt = ids[i];
    
    //free(values);
    //free(ids);

    //printf("valor final %lf com id dos ids %ld e id %ld\n", max_dist, i, idx_newpt);
    return idx_newpt;
}
